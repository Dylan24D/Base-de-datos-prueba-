...
// ============================================
let chatHistory = JSON.parse(sessionStorage.getItem('gengar_chat_history')) || [];
...
function clearChat() {
    if (confirm("¿Deseas limpiar el historial espectral?")) {
        sessionStorage.clear(); // Limpia todo el sessionStorage, incluyendo el historial
        chatHistory = [];
        initChat();
    }
}
...
function renderChatMessage(text, sender, save = true) {
    ...
    if (save) {
        ...
        sessionStorage.setItem('gengar_chat_history', JSON.stringify(chatHistory));
...
function renderChatOptions(options, save = true) {
    ...
    if (save) {
        ...
        sessionStorage.setItem('gengar_chat_history', JSON.stringify(chatHistory)); // Guardar opciones también
    }
}
